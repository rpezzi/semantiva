...
for analyzing Semantiva pipelines. The builder constructs detailed inspection
...
class PipelineInspection:
    """Complete inspection results for an entire pipeline.
...
def build_pipeline_inspection(
    pipeline_or_config: Union[Pipeline, List[Dict[str, Any]]],
) -> PipelineInspection:
    """Build a :class:`PipelineInspection` from a pipeline or configuration.

    This function is the core of the inspection system. It analyzes either a
    constructed Pipeline object or raw configuration data to produce detailed
    ...
    Args:
        pipeline_or_config: Either a constructed Pipeline object or a list
            of node configuration dictionaries
    ...
    Implementation Notes:
        - For Pipeline objects, uses existing nodes and configuration
        - For raw config, attempts to construct nodes via node factory
    ...
    # Determine if we're working with a pipeline or raw configuration
    if isinstance(pipeline_or_config, Pipeline):
        node_configs = pipeline_or_config.pipeline_configuration
        nodes = pipeline_or_config.nodes
    else:
        node_configs = pipeline_or_config
        nodes = []
    ...
    for index, node_cfg in enumerate(node_configs, start=1):
        ...
        # Get or create the pipeline node
        if isinstance(pipeline_or_config, Pipeline):
            # Use existing node from constructed pipeline
            node = nodes[index - 1]
        else:
